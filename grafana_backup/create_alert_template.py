...
def main(args, settings, file_path):
    ...
    if minimum_version <= grafana_version:
        ...
        result = update_alert_template(json.dumps(template), name, grafana_url, http_post_headers, verify_ssl,
                                       client_cert,
                                       debug)
        print("create alert template: {0}, status: {1}, msg: {2}".format(template['name'], result[0], result[1]))
    else:
