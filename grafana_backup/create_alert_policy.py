...
def main(args, settings, file_path):
    ...
    if minimum_version <= grafana_version:
        alert_policy = json.loads(data)
        http_post_headers['x-disable-provenance'] = '*'
        result = update_alert_policies(json.dumps(alert_policy), grafana_url, http_post_headers, verify_ssl,
                                       client_cert, debug)
        print("create alert policy: {0}, status: {1}, msg: {2}".format(alert_policy['receiver'], result[0], result[1]))
    else:
        print("Unable to create alert policy, requires Grafana version {0} or above. Current version is {1}".format(
